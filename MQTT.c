...
//					lastOutActivity = TickGet();	// già in write
		...
		case MQTT_CONNECT_ACK:
			...
			while(!MQTTAvailable()) {
				WORD t = TickGet();
//								theApp.PumpMessage();
				if(t-lastInActivity > MQTTClient.KeepAlive*1000UL) {
					MQTTStop();
					MQTTState=MQTT_IDLE;
					break;
					}
...
//		case MQTT_LOOP:	
			...
			if(MQTTConnected()) {
				...
				if((t - lastInActivity > MQTTClient.KeepAlive*1000UL) || (t - lastOutActivity > MQTTClient.KeepAlive*1000UL)) {
...
BOOL MQTTWrite(BYTE header, BYTE *buf, WORD length) {
  ...
  do {
	...
	if(TCPIsPutReady(length+1+llen)) {
		rc = MQTTPutArray(buf+(4-llen),length+1+llen);
	  lastOutActivity = TickGet();
...
WORD MQTTReadPacket(BYTE *lengthLength) {
// qua  leggermente diversa...	https://community.spark.io/t/mqtt-library-and-sample/2111
	WORD len = 0;
	...
	switch(m_state) {
		case 0:
			if(TCPIsGetReady(4))				// almeno 4 ci devono essere...
				m_state++;
			break;
		...
		case 2:
			if(isPublish) {
				if(TCPIsGetReady(2)) {			// almeno 2 ci devono essere... 
		...
		case 3:
			if(TCPIsGetReady(length)) {
				for(i=start; i<length; i++) {
					digit = MQTTReadByte();
					if(MQTTClient.Stream) {
						if(isPublish && len-*lengthLength-2>skip) {
			//        MQTTClient.Stream->Write((char *)digit,strlen((char *)digit));		//verificare che faceva...
							}
						}
